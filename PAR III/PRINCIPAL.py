...
from update_precio import update_precio
...
from CONTRASEÑAS import CONTRA_ADMIN,CONTRA_COLAB
...
class UsuariosScreen(Screen):
    pass
class AdministradorScreen(Screen):
    def a(self):
        self.ids.admicontra.text=''
        
class ColaboradorScreen(Screen):
    def a(self):
        self.ids.colabcontra.text=''
class MenuScreen(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

class MenuScreenColab(Screen):
    pass
...
class Actualizar_Stock(Screen):
    ...
    def consultar_nombre(self):
        try:
            ...
            self.stock_disponible = str(tupladatos[4])
            self.ids.salidanombre.text = self.nombre
            self.ids.salidastock.text = self.stock
            self.ids.salidastockdisponible.text = self.stock_disponible
        except:
            self.nombre=''
    ...
    def Sumar_Stock(self):
        ...
        e = self.ids.salidastockdisponible.text
        ee = int(e)
        aa = int(a)
        self.bb = int(b)
        self.dd = d.upper()
        ...
        self.f = ee + self.bb
        cc = str(self.c)
        update_stock(self.dd, self.c,self.f)

    def Limpiar(self):
        self.ids.ingresostock.text = ''
        self.ids.entradacodigo.text=''
        self.ids.salidanombre.text='NOMBRE DEL PROD.'
        self.ids.salidastock.text='STOCK ACTUAL'
        self.ids.salidastockdisponible.text='STOCK DISPONIBLE'
...
class Agregar_Producto(Screen):
    def __init__(self, **kwargs):
        ...
        for i in range(0,len(tupla_cod)):
            self.lista_cod.append(tupla_cod[i])
...
class Cambiar_Precio(Screen):
    def consultar_precio(self):
        try:
            a = self.ids.entradacodigop.text
            self.b=(str(a)).upper()
            tupladatos = select_inventario(self.b)
            self.nombre = str(tupladatos[0])
            self.precio = str(tupladatos[1])
            self.ids.salidanombrep.text = self.nombre
            self.ids.salidaprecio.text = f"S/. {self.precio}"
        except:
            self.nombre=''
            #self.ids.entradacodigo.text =''
            self.ids.salidanombrep.text = 'NO ENCONTRADO'
            self.ids.salidaprecio.text = 'NO ENCONTRADO'

    def Cambiar_precio(self):
        a = self.ids.nuevoprecio.text
        aa = float(a)
        update_precio(self.b,aa)


    def Limpiar2(self):
        self.ids.nuevoprecio.text = ''
        self.ids.entradacodigop.text=''
        self.ids.salidanombrep.text='NOMBRE DEL PROD.'
        self.ids.salidaprecio.text='PRECIO ACTUAL'
...
class Pop2(Popup):
    def __init__(self,Cp):
        ...
        self.Cp = Cp
        self.Llenar2()
    def Llamar_Limpiar2(self):
        self.Cp.nombre
        if type(self.Cp) == Cambiar_Precio:
            self.Cp.Limpiar2()
        #nombre producto(self.As.nombre)
        #cant. ingresada entero(self.As.bb)
        #codigo en mayusculas (self.As.dd)
    ...
    def Llenar2(self):
        total = self.Cp.ids.nuevoprecio.text
        self.ids.vpnombrep.text = self.Cp.nombre
        self.ids.vpnprecio.text = f"S/. {str(total)}"
...
class BoletaScreen(Screen):
    ...
    def Llenar_Boletas(self,i):
        ...
        try:
            ...
            self.lista_stock[i].text = str(li1[4])
    ...
    def Calcular_Total(self):
        ...
        for i in range(5):
            self.total += round(float(self.lista_s[i]),2)
        self.ids.total.text = f'S/. {self.total}'   
    ...
    def Restar_Ventas_Base(self,x2):
        ...
        try:
            ...
            for i in range (0,len(self.lista_s)):
                ...
                stock = int((select_inventario(cod))[2])
                stockD = int(self.lista_stock[i].text)
                cant_vendida = int(self.lista_cantidad[i].text)
                update_stock(cod,(stock-cant_vendida),(stockD-cant_vendida))
...
class VENTANA_PRINCIPALApp(App):
    ...
    def build(self):
        ...
        sm.add_widget(UsuariosScreen(name='usu'))
        sm.add_widget(AdministradorScreen(name='admipass'))
        sm.add_widget(ColaboradorScreen(name='colabpass'))

        sm.add_widget(MenuScreen(name='menu'))
        sm.add_widget(MenuScreenColab(name='menuc'))
        ...
        sm.add_widget(Cambiar_Precio(name='cprecio'))

        return sm
        
    def validaradmi(self,contra):
        if contra == CONTRA_ADMIN:
            self.root.current = 'menu'
        else:
            self.Advertir()

    def validarcolab(self,contra):
        if contra == CONTRA_COLAB:
            self.root.current = 'menuc'
        else:
            self.Advertir()
    ...
    def Advertir(self):
        ...
        boxcontenedor = BoxLayout(orientation = 'vertical')
        lbl_again1 = Label(text = 'CONTRASEÑA INCORRECTA, INTENTE NUEVAMENTE')
        bot_volver = Button(text = 'INTENTAR NUVEAMENTE')
        bot_volver.bind(on_release = self.cerrar_pop)
        boxcontenedor.add_widget(lbl_again1)
        boxcontenedor.add_widget(bot_volver)
